...
$sql = "CREATE TABLE IF NOT EXISTS mushroom (seller_name TEXT NOT NULL, stock INTEGER NOT NULL, unit_price NUMERIC(10,2) NOT NULL)";
...
//create oil table
$sql = "CREATE TABLE IF NOT EXISTS oil (seller_name TEXT NOT NULL, stock INTEGER NOT NULL, unit_price NUMERIC(10,2) NOT NULL)";
$result = pg_query($db, $sql);
if (!$result) {
    die('Error: ' . pg_last_error());
}
...
if ($product == 'mushroom') {
    $sql = "INSERT INTO mushroom(seller, quantity, unit_price) VALUES ('$seller', $stock, $price)";
    $result = pg_query($db, $sql);
    if (!$result) {
        die('Error: ' . pg_last_error());
    }
} elseif ($product == 'oil') {
    $sql = "INSERT INTO oil(seller, quantity, unit_price) VALUES ('$seller', $stock, $price)";
    $result = pg_query($db, $sql);
    if (!$result) {
        die('Error: ' . pg_last_error());
    }
...
$response = "Thank you for your purchase of $quantity kilograms of $product. Your total cost is $price"
